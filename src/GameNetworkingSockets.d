...
// the relevant parts of steamtypes.h
/+
alias GID_t = Typedef!(ulong, ulong.init, "GID");
alias JobID_t = Typedef!(ulong, ulong.init, "JobID");
alias TxnID_t = Typedef!(GID_t, GID_t.init, "TxnID");
alias PackageId_t = Typedef!(uint, uint.init, "PackageID");
alias BundleId_t = Typedef!(uint, uint.init, "BundleID");

enum GID_t k_GIDNil = 0xffffffffffffffff;

enum TxnID_t k_TxnIDNil = cast(TxnID_t)k_GIDNil;
enum TxnID_t k_TxnIDUnknown = 0;

enum PackageId_t k_uPackageIdFreeSub = 0x0;
enum PackageId_t k_uPackageIdInvalid = 0xFFFFFFFF;

enum BundleId_t k_uBundleIdInvalid = 0;
...
extern (C) {
	...
	enum NetConnection NetConnection_Invalid = 0;
	enum ListenSocket ListenSocket_Invalid = 0;
	...
	enum ConnectionState {
		...
		// under the hood.
		//
		// as such, they are left into the d binding, but prefixed with an underscore --Elronnd
